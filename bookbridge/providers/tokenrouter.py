"""TokenRouter OpenAI-compatible provider implementation."""
...
class TokenRouterProvider(TranslationProvider):
    ...
    def resolve_model_name(self, model_name: str | None) -> str:
        ...
        if cleaned.lower().startswith("tokenrouter/"):
            cleaned = cleaned[12:]
            ...
            try:
                ...
                if status == 200:
                    ...
                    if choices:
                        ...
                        return TranslationResult(
                            ...
                            source_text=text,
                            provider=ProviderType.TOKENROUTER.value,
                            model=model,
                    ...
                    return TranslationResult(
                        ...
                        model=model,
                        credential_id=credential.id,
                        error="TokenRouter returned empty choices.",
                        retryable=False,
                    )
                ...
                if status == 429:
                    return TranslationResult(
                        ...
                        model=model,
                        credential_id=credential.id,
                        error="HTTP 429: TokenRouter rate limit exceeded.",
                        retryable=True,
                    )
                if status in (401, 403):
                    return TranslationResult(
                        ...
                        model=model,
                        credential_id=credential.id,
                        error=f"HTTP {status}: Invalid TokenRouter API key.",
                        retryable=False,
                    )
                return TranslationResult(
                    success=False,
                ...
                return TranslationResult(
                    ...
                    model=model,
                    credential_id=credential.id,
                    error="TokenRouter request timeout.",
                    retryable=True,
                )
                ...
                return TranslationResult(
                    ...
                    model=model,
                    credential_id=credential.id,
                    error=f"TokenRouter client error: {str(ex)}",
                    retryable=True,
                )
